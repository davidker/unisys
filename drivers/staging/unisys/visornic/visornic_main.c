	      ...
	      struct sk_buff *skb, struct net_device *netdev)
{
	...
	if (ULTRA_IO_USING_ALIGNED_IOCHANNEL_STRUCTS(features)) {
		printk(KERN_ERR "iochannel_structs: using aligned");
		rcv_done_len = copy.rcv.rcv_done_len;
		numrcvbufs = copy.rcv.numrcvbufs;
		rcvbuf = copy.rcv.rcvbuf;
	}
	else {
		printk(KERN_ERR "iochannel_structs: using unaligned");
...
static int visornic_probe(struct visor_device *dev)
{
	...
	features |= ULTRA_IO_DRIVER_SUPPORTS_ALIGNED_IOCHANNEL_STRUCTS;
	...
	visorbus_set_channel_features(dev, ULTRA_IO_DRIVER_SUPPORTS_ALIGNED_IOCHANNEL_STRUCTS);
