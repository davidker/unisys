...
static int client_bus_info_debugfs_show(struct seq_file *seq, void *v)
{
	...
	if (visorchannel_read(channel,
			      offsetof(struct visor_vbus_channel_protocol,
				       chp_info),
	...
	if (visorchannel_read(channel,
			      ...
			      &dev_info, sizeof(dev_info)) >= 0)
		vbuschannel_print_devinfo(&dev_info, seq, -1);
	off = offsetof(struct visor_vbus_channel_protocol, dev_info);
	i = 0;
		     ...
		     struct visor_vbus_headerinfo *hdr_info)
{
	...
	if (!spar_check_channel(visorchannel_get_header(chan),
				...
				"vbus",
				sizeof(struct visor_vbus_channel_protocol),
				VISOR_VBUS_CHANNEL_PROTOCOL_VERSIONID,
				VISOR_VBUS_CHANNEL_PROTOCOL_SIGNATURE))
		return -EINVAL;
 ...
 *                         visor_vbus_channel_protocol.chp_info
 ...
 *                         visor_vbus_channel_protocol.bus_info
 * @chan:     indentifies the s-Par channel that will be updated
 * @hdr_info: used to find appropriate channel offset to write data
 * @info:     contains the information to write
 *
 ...
 *                         visor_vbus_channel_protocol.dev_info[<devix>]
 * @chan:     indentifies the s-Par channel that will be updated
 * @hdr_info: used to find appropriate channel offset to write data
 * @info:     contains the information to write
...
/*
 * create_visorbus_instance() - create a device instance for the visorbus itself
 ...
 */
static int
create_visorbus_instance(struct visor_device *dev)
{
...
err_debugfs_dir:
	debugfs_remove_recursive(dev->debugfs_dir);
	kfree(hdr_info);
	dev_err(&dev->device, "create_visorbus_instance failed: %d\n", err);
	return err;
}

/*
 * remove_visorbus_instance() - remove a device instance for the visorbus itself
 * @dev: struct visor_device indentifying the bus to remove
 */
static void
remove_visorbus_instance(struct visor_device *dev)
{
	/*
...
visorchipset_bus_create(struct visor_device *dev)
{
	...
	err = create_visorbus_instance(dev);
...
/*
 * initiate_visorchipset_device_pause_resume() - start a pause or resume
 ...
 */
static int
initiate_visorchipset_device_pause_resume(struct visor_device *dev,
					  bool is_pause)
{
	int err;
	struct visor_driver *drv = NULL;
...
visorchipset_device_pause(struct visor_device *dev_info)
{
	...
	err = initiate_visorchipset_device_pause_resume(dev_info, true);
...
visorchipset_device_resume(struct visor_device *dev_info)
{
	...
	err = initiate_visorchipset_device_pause_resume(dev_info, false);
