	   ...
	   int response, bool need_response)
{
	...
	if (need_response) {
		...
		if (!pmsg_hdr) {
			visorbus_log_postcode(CURRENT_FILE_PC,
					      MALLOC_FAILURE_PC, __LINE__, cmd,
 ...
 * to enable or disable. The CONTROLVM message handler calls parahotplug
 * process_message, which then adds the message to a global list and kicks
 * off a udev event which causes a user level script to enable or disable
 * the specified device. The udev script then writes to
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("s-Par visorchipset/visorbus driver for virtual device busses");
